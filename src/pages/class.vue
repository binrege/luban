...
<!-- 循环数组 -->
				<div class="c" v-for="(item,index) in imgs" :key="index" >
					<div class="top">
						<dl>
							<dt><div><a href="#" target="_self" ><img :src="item.courseImage" ></a></div><img :src="item.courseImage" ></dt>
							<dd><!--{{item.t.list.courseImage}}-->
							<br/><span>vip课程系列</span>
							</dd>
						</dl>
					</div>
					<div class="bottom">
...
let Base64 = require("js-base64").Base64;
 export default {
   ...
   mounted() {
      this.findCourse()  //调用方法 ,记到加this
   },
   methods: {
      findCourse(){ //自己命名  ,对应上面的数据
      this.$api  //不变
        .findCourse()  //接口的名字
        .then(res => {
          // console.log(JSON.parse(Base64.decode(res)))
          console.log(res)
          this.imgs = res.t.list;
        })
        .catch(err => {
          console.log(err);
        });
